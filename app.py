...
def get_organization_members(org_id, people_data):
    ...
    today = datetime.strptime("2025-09-23", "%Y-%m-%d")
        ...
        if 'membership' in person:
            ...
            for membership in memberships:
                ...
                if end_date:
                    ...
                    if end_dt > today:
                        continue  # Mitgliedschaft läuft noch nicht oder endet nach 23.10.2025
...
if org_response and people_response:
    ...
    for org in organizations:
        ...
        if end_date:
            end_dt = datetime.fromisoformat(end_date[:10])
            if end_dt > today:
                continue  # Ausschuss läuft nach 23.10.2025
    ...
    if committee_stats:
        ...
        with col1:
            ...
            fig = go.Figure()
            fig.add_trace(go.Bar(name='Männer', y=df['Name'], x=df['Männer'], orientation='h', marker_color='#4A90E2', text=df['Männer'], textposition='inside'))
            fig.add_trace(go.Bar(name='Frauen', y=df['Name'], x=df['Frauen'], orientation='h', marker_color='#E94B8B', text=df['Frauen'], textposition='inside'))
            if df['Unbekannt'].sum() > 0:
                fig.add_trace(go.Bar(name='Unbekannt', y=df['Name'], x=df['Unbekannt'], orientation='h', marker_color='#CCCCCC', text=df['Unbekannt'], textposition='inside'))
            fig.update_layout(barmode='stack', height=max(400, len(df) * 40), xaxis_title="Anzahl Mitglieder", yaxis_title="", legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1), margin=dict(l=0, r=0, t=30, b=0))
        ...
        with col2:
            ...
            fig_pie = go.Figure(data=[go.Pie(labels=['Männer', 'Frauen'], values=[total_men, total_women], marker_colors=['#4A90E2', '#E94B8B'], hole=0.4)])
            fig_pie.update_layout(title="Gesamtverteilung", height=300, margin=dict(l=0, r=0, t=40, b=0))
        ...
        st.dataframe(df_display, use_container_width=True, hide_index=True, column_config={
            ...
            "Frauenanteil %": st.column_config.ProgressColumn("Frauenanteil", format="%.1f%%", min_value=0, max_value=100)
        })
        
        csv = df.to_csv(index=False, encoding='utf-8-sig')
        st.download_button(label="📥 Daten als CSV herunterladen", data=csv, file_name="ausschuesse_grevenbroich_2025-2030.csv", mime="text/csv")
